...
#define HW_TX_TS_MODE_MASK 0b0000110          // Mask to get if device has needed HW TS TX modes.
#define HW_RX_FILTER_MASK 0b1100000000000000  // Mask for RX filters.

enum class PTPMechanism {
    NONE,
    LINUXPTP,
    EXCELFORE_GPTP
};

static std::string mechanism_to_string(const PTPMechanism& mechanism) {
    switch (mechanism) {
        case PTPMechanism::NONE:
            return "none";
        case PTPMechanism::LINUXPTP:
            return "linuxptp";
        case PTPMechanism::EXCELFORE_GPTP:
            return "excelfore-gptp";
    }
}
...
public:
    ...
    [[nodiscard]] bool is_applying_on_system() const {
        return _applying_on_system;
    }
    [[nodiscard]] long master_offset() const {
        return _master_offset;
    }
    [[nodiscard]] long ingress_time() const {
        return _ingress_time;
    }
    ...
    [[nodiscard]] bool is_grand_master_present() const {
        return _grand_master_present;
    }
...
#ifdef _EXCELFORE_GPTP
    bool _running = true;
    PTPMechanism _mechanism = PTPMechanism::EXCELFORE_GPTP;
#else
    bool _running = stat("/var/run/ptp4l", &buffer) == 0;
    PTPMechanism _mechanism = _running ? PTPMechanism::LINUXPTP : PTPMechanism::NONE;
#endif
