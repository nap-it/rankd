...
#ifndef RANKD_IPERF_H
#define RANKD_IPERF_H
...
class IperfSubListener : public DataReaderListener {
public:
    IperfSubListener() {}

    ~IperfSubListener() override {}
    ...
    Iperf::IperfStatus _message;
};

class IperfSubscriber {
public:
    IperfSubscriber() : _participant(nullptr), _subscriber(nullptr), _reader(nullptr), _topic(nullptr) {}

    virtual ~IperfSubscriber() {
        if (_reader != nullptr) {
    ...
    void share(Iperf::IperfStatus* data, std::mutex* mutex) {
        _data = data;
    ...
    bool execute() {
        ...
        _topic = _participant->create_topic("IperfStatusTopic", "Iperf::IperfStatus", TOPIC_QOS_DEFAULT);
        if (_topic == nullptr) {
            return false;
        }
