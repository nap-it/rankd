...
int nl_dump_ext_ack(const struct nlmsghdr *nlh, nl_ext_ack_fn_t errfn) {
  ...
  if (tb[NLMSGERR_ATTR_OFFS]) {
    ...
    if (off > nlh->nlmsg_len) {
      fprintf(stderr, "Invalid offset for NLMSGERR_ATTR_OFFS\n");
      off = 0;
...
int nl_dump_ext_ack_done(const struct nlmsghdr *nlh, unsigned int offset,
                         int error) {
  struct nlattr *tb[NLMSGERR_ATTR_MAX + 1] = {};
  const char *msg = NULL;
...
void rtnl_set_strict_dump(struct rtnl_handle *rth) {
  ...
  if (setsockopt(rth->fd, SOL_NETLINK, NETLINK_GET_STRICT_CHK, &one,
                 sizeof(one)) < 0)
    return;
...
int rtnl_add_nl_group(struct rtnl_handle *rth, unsigned int group) {
  return setsockopt(rth->fd, SOL_NETLINK, NETLINK_ADD_MEMBERSHIP, &group,
                    sizeof(group));
}
...
int rtnl_nexthopdump_req(struct rtnl_handle *rth, int family,
  ...
  struct custom_struct req {};
  req.nlh.nlmsg_len = NLMSG_LENGTH(sizeof(struct nhmsg));
  req.nlh.nlmsg_type = RTM_GETNEXTHOP;
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.nhm.nh_family = static_cast<unsigned char>(family);
                                ...
                                req_filter_fn_t filter_fn)
{
 struct {
   ...
   char buf[128];
     ...
     .nhm.nh_family = static_cast<unsigned char>(family),
 };
...
int rtnl_addrdump_req(struct rtnl_handle *rth, int family,
  ...
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.ifm.ifa_family = static_cast<__u8>(family);
...
int rtnl_addrlbldump_req(struct rtnl_handle *rth, int family) {
  ...
  struct custom_struct req{};
  req.nlh.nlmsg_len = NLMSG_LENGTH(sizeof(struct ifaddrlblmsg));
  req.nlh.nlmsg_type = RTM_GETADDRLABEL;
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.ifal.ifal_family = static_cast<__u8>(family);
...
int rtnl_ruledump_req(struct rtnl_handle *rth, int family) {
  ...
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.frh.family = static_cast<__u8>(family);
...
int rtnl_neighdump_req(struct rtnl_handle *rth, int family,
  ...
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.ndm.ndm_family = static_cast<__u8>(family);
...
int rtnl_neightbldump_req(struct rtnl_handle *rth, int family) {
  ...
  struct custom_struct req{};
  req.nlh.nlmsg_len = NLMSG_LENGTH(sizeof(struct ndtmsg));
  req.nlh.nlmsg_type = RTM_GETNEIGHTBL;
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.ndtmsg.ndtm_family = static_cast<__u8>(family);
...
int rtnl_mdbdump_req(struct rtnl_handle *rth, int family) {
  ...
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.bpm.family = static_cast<__u8>(family);
...
int rtnl_brvlandump_req(struct rtnl_handle *rth, int family, __u32 dump_flags) {
  ...
  struct custom_struct req{};
  req.nlh.nlmsg_len = NLMSG_LENGTH(sizeof(struct br_vlan_msg));
  req.nlh.nlmsg_type = RTM_GETVLAN;
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.bvm.family = static_cast<__u8>(family);
...
int rtnl_netconfdump_req(struct rtnl_handle *rth, int family) {
  ...
  struct custom_struct req{};
  req.nlh.nlmsg_len = NLMSG_LENGTH(NLMSG_ALIGN(sizeof(struct netconfmsg)));
  req.nlh.nlmsg_type = RTM_GETNETCONF;
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.ncm.ncm_family = static_cast<__u8>(family);
...
int rtnl_nsiddump_req_filter_fn(struct rtnl_handle *rth, int family,
  ...
  struct custom_struct req{};
  req.nlh.nlmsg_len = NLMSG_LENGTH(NLMSG_ALIGN(sizeof(struct rtgenmsg)));
  req.nlh.nlmsg_type = RTM_GETNSID;
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.rtm.rtgen_family = static_cast<unsigned char>(family);
...
static int __rtnl_linkdump_req(struct rtnl_handle *rth, int family) {
  ...
  req.nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST;
  req.nlh.nlmsg_seq = rth->dump = ++rth->seq;
  req.ifm.ifi_family = static_cast<unsigned char>(family);
...
int rtnl_dump_request(struct rtnl_handle *rth, int type, void *req, int len) {
  struct nlmsghdr nlh = {
      .nlmsg_len = static_cast<__u32>(NLMSG_LENGTH(len)),
      .nlmsg_type = static_cast<__u16>(type),
  ...
  struct iovec iov[2] = {
      ...
      {.iov_base = req, .iov_len = static_cast<size_t>(len)}};
  struct msghdr msg = {
      .msg_name = &nladdr,
...
static int rtnl_dump_done(struct nlmsghdr *h,
  ...
  if (len < 0) {
    ...
    if (a->errhndlr &&
        (a->errhndlr(h, a->arg2) & RTNL_SUPPRESS_NLMSG_DONE_NLERR))
      return 0;
...
static int rtnl_dump_error(const struct rtnl_handle *rth, struct nlmsghdr *h,
                           const struct rtnl_dump_filter_arg *a) {
  ...
  if (h->nlmsg_len < NLMSG_LENGTH(sizeof(struct nlmsgerr))) {
    ...
    if (a->errhndlr &&
        (a->errhndlr(h, a->arg2) & RTNL_SUPPRESS_NLMSG_ERROR_NLERR))
      return 0;
...
int rtnl_dump_filter_errhndlr_nc(struct rtnl_handle *rth, rtnl_filter_t filter,
                                 void *arg1, rtnl_err_hndlr_t errhndlr,
                                 void *arg2, __u16 nc_flags) {
  const struct rtnl_dump_filter_arg a[] = {
      {
          .filter = filter,
          .arg1 = arg1,
...
static int __rtnl_talk_iov(struct rtnl_handle *rtnl, struct iovec *iov,
  ...
  for (i = 0; i < iovlen; i++) {
    h = static_cast<nlmsghdr *>(iov[i].iov_base);
  ...
  next:
    ...
    for (h = (struct nlmsghdr *)buf; status >= sizeof(*h);) {
      ...
      if (nladdr.nl_pid != 0 || h->nlmsg_pid != rtnl->local.nl_pid ||
...
int rtnl_listen(struct rtnl_handle *rtnl, rtnl_listen_filter_t handler,
                void *jarg) {
  int status;
  struct nlmsghdr *h;
  struct sockaddr_nl nladdr = {.nl_family = AF_NETLINK};
...
int rtnl_from_file(FILE *rtnl, rtnl_listen_filter_t handler, void *jarg) {
  size_t status;
  char buf[16384];
  struct nlmsghdr *h = (struct nlmsghdr *)buf;
...
int addattr_nest_compat_end(struct nlmsghdr *n, struct rtattr *start) {
  struct rtattr *nest = static_cast<struct rtattr *>(
      (void *)((char *)start + NLMSG_ALIGN(start->rta_len)));
  ...
  rtattr_for_each_nested(pos, attr) {
    ...
    rtattr_for_each_nested(attr, pos) {
      ...
      if (tp[NL_POLICY_TYPE_ATTR_TYPE])
        fprintf(
            fp, "attr[%u]: type=%s", attr->rta_type & ~NLA_F_NESTED,
      ...
      if (tp[NL_POLICY_TYPE_ATTR_MIN_VALUE_S] &&
        ...
        fprintf(fp, " range:[%lld,%lld]",
                (signed long long)rta_getattr_u64(
                    tp[NL_POLICY_TYPE_ATTR_MIN_VALUE_S]),
                (signed long long)rta_getattr_u64(
                    tp[NL_POLICY_TYPE_ATTR_MAX_VALUE_S]));
      ...
      if (tp[NL_POLICY_TYPE_ATTR_MIN_VALUE_U] &&
        ...
        fprintf(fp, " range:[%llu,%llu]",
                (unsigned long long)rta_getattr_u64(
                    tp[NL_POLICY_TYPE_ATTR_MIN_VALUE_U]),
                (unsigned long long)rta_getattr_u64(
                    tp[NL_POLICY_TYPE_ATTR_MAX_VALUE_U]));
                       ...
                       __u8 flags)
{
 struct {
   ...
   char buf[256];
     ...
     .nlh.nlmsg_flags = NLM_F_DUMP | NLM_F_REQUEST,
     .nlh.nlmsg_seq = rth->dump = ++rth->seq,
     .tmsg.family = static_cast<__u8>(family),
     .tmsg.flags = flags,
     .tmsg.ifindex = static_cast<__u32>(ifindex),
 };
