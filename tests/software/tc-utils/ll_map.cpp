                                       ...
                                       struct ll_cache *parent_im)
{
 ...
 im = static_cast<ll_cache *>(malloc(sizeof(*im) + strlen(ifname) + 1));
 if (!im)
   return NULL;
                                     ...
                                     struct ifinfomsg *ifi, struct rtattr **tb)
{
 ...
 for (i = static_cast<struct rtattr*>(RTA_DATA(proplist)); RTA_OK(i, rem);
      i = RTA_NEXT(i, rem)) {
...
static int ll_link_get(const char *name, int index)
{
 ...
 if (!rc) {
   struct ifinfomsg *ifm = static_cast<struct ifinfomsg*>(NLMSG_DATA(answer));
